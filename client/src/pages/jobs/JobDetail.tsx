...
const coverLetterText: Record<CoverLetterPolicy, string> = {
  required: 'Cover letter required',
  optional: 'Cover letter optional',
  none: 'No cover letter needed',
}
    ...
    return (
      <section className="px-4 py-20 sm:px-8 sm:py-28">
        <div className="mx-auto max-w-xl text-center">
          <h1 className="text-3xl font-medium tracking-[-.05em] sm:text-5xl">{(error as { status?: number })?.status === 404 ? 'This position is no longer open.' : 'We couldn’t load this position.'}</h1>
          <p className="mt-4 text-foreground/60">{error?.message}</p>
          <Link to="/jobs" className={cn(buttonVariants(), 'mt-8 rounded-md bg-brand text-brand-foreground hover:bg-brand-hover')}>Browse open positions</Link>
        </div>
      </section>
    )
  }
  ...
  return (
    <article className="px-4 py-10 sm:px-8 sm:py-14">
      <div className="mx-auto max-w-7xl">
        <Link to="/jobs" className="inline-flex items-center gap-1 text-sm font-semibold text-brand"><ArrowLeft size={15} aria-hidden /> All positions</Link>
        ...
        <header className="mt-6 flex flex-col gap-6 border-b border-foreground/10 pb-8 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <ul className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-foreground/65">
              ...
              <li className="flex items-center gap-1.5"><Clock size={15} aria-hidden /> Posted {timeAgo(job.createdAt)}</li>
            </ul>
          </div>
          <Link to={applyHref} className={cn(buttonVariants({ size: 'lg' }), 'h-11 shrink-0 rounded-md bg-brand px-6 text-brand-foreground hover:bg-brand-hover')}>
            Apply now <ArrowRight data-icon="inline-end" aria-hidden />
          </Link>
        </header>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_340px]">
          <div className="min-w-0">
            ...
            <section className="mt-10" aria-labelledby="skills-heading">
              <h2 id="skills-heading" className="text-xl font-semibold tracking-[-.02em]">Skills</h2>
              <p className="mt-4 text-xs font-semibold uppercase tracking-[.14em] text-foreground/50">Required</p>
              ...
              {job.niceToHaveSkills.length > 0 && (
                <>
                  <p className="mt-5 text-xs font-semibold uppercase tracking-[.14em] text-foreground/50">Nice to have</p>
                  <div className="mt-2 flex flex-wrap gap-2">
          ...
          <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
            <div className="rounded-lg border border-foreground/12 bg-surface p-6">
              <p className="text-xs font-semibold uppercase tracking-[.14em] text-foreground/50">Salary</p>
              ...
              <dl className="mt-5 space-y-3 border-t border-foreground/10 pt-5 text-sm">
                {job.languages.length > 0 && (
                  <div className="flex gap-2.5">
                    <dt><Languages size={16} className="mt-0.5 text-foreground/50" aria-label="Languages" /></dt>
                    <dd>{job.languages.map((l) => `${l.name} (${proficiencyLabel[l.proficiency]})`).join(', ')}</dd>
                  </div>
                )}
                <div className="flex gap-2.5">
                  <dt><FileText size={16} className="mt-0.5 text-foreground/50" aria-label="Cover letter" /></dt>
                  <dd>{coverLetterText[job.coverLetterPolicy]}</dd>
                </div>
              </dl>
              <Link to={applyHref} className={cn(buttonVariants({ size: 'lg' }), 'mt-6 h-11 w-full rounded-md bg-brand text-brand-foreground hover:bg-brand-hover')}>Apply now</Link>
              <p className="mt-3 text-center text-xs text-foreground/55">Your contact details stay private until you accept an introduction.</p>
            </div>

            <div className="rounded-lg border border-foreground/12 p-6">
              <p className="flex items-center gap-2 font-semibold"><Building2 size={17} className="text-brand" aria-hidden /> About {job.company.name}</p>
              ...
              <dl className="mt-4 space-y-1.5 text-sm text-foreground/65">
                {job.company.industry && <div><dt className="inline">Industry: </dt><dd className="inline">{job.company.industry}</dd></div>}
                {job.company.size && <div><dt className="inline">Size: </dt><dd className="inline">{job.company.size} employees</dd></div>}
              </dl>
              {job.company.website && (
                <a href={job.company.website} target="_blank" rel="noopener noreferrer nofollow" className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-brand">
                  Website <ExternalLink size={14} aria-hidden /><span className="sr-only">(opens in a new tab)</span>
                </a>
              )}
            </div>
          </aside>
