...
export function QuickApply({ job, onDone }: { job: JobDetail; onDone: (r: ApplySubmitResult) => void }) {
  ...
  const submit = () => {
    if (job.coverLetterPolicy === 'required' && !text.trim() && !file) return setError('This position requires a cover letter: write one or upload a file')
    setError(undefined)
    apply.mutate()
  }
  ...
  return (
    <section className="px-4 py-10 sm:px-8 sm:py-14">
      <div className="mx-auto max-w-2xl">
        <p className="mb-3 text-xs font-semibold uppercase tracking-[.14em] text-brand">Apply with your profile</p>
          ...
          {isPending || !profile ? (
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 rounded-md bg-chip p-4">
              <div>
                ...
                <p className="text-sm text-foreground/65">{profile.headline} · {profile.totalYearsExperience} years</p>
              ...
              <Link to="/candidate/profile" className="text-sm font-semibold text-brand">Update profile first</Link>
            </div>
          )}

          {job.coverLetterPolicy !== 'none' && (
            <div className="space-y-4">
              <p className="text-sm font-semibold">Cover letter <span className="font-normal text-foreground/55">{job.coverLetterPolicy === 'required' ? '(required)' : '(optional)'}</span></p>
              <Field label="Write it here" error={error}>
                {(ids) => <Textarea {...ids} rows={6} value={text} onChange={(e) => setText(e.target.value)} placeholder="Why this role, and what would you bring?" />}
              </Field>
              <FileDrop label="…or upload a file" file={file} onChange={setFile} />
            </div>
          )}

          {apply.error && (
            <Alert variant={alreadyApplied ? 'info' : 'error'}>
              {apply.error.message}. {alreadyApplied && <Link to="/candidate/applications" className="font-semibold underline">See your applications</Link>}
            </Alert>
          )}
