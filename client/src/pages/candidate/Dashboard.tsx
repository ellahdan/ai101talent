...
export default function CandidateDashboard() {
  ...
  if (!profile.data) return <><PageHeader title="Welcome!" /><NoProfile /></>
  ...
  return (
    <>
      <PageHeader title={`Hi, ${p.fullName.split(' ')[0]}`} description="Here's where your applications and introductions stand." />

      <div className="grid gap-4 md:grid-cols-[1.2fr_1fr_1fr]">
        <div className="rounded-lg bg-ink p-6 text-ink-foreground">
          <p className="text-xs font-semibold uppercase tracking-[.14em] text-brand-bright">Applicant number</p>
        ...
        <Stat label="Active applications" value={applications.isPending ? '…' : String(active)} icon={ListChecks} to="/candidate/applications" />
        <Stat label="Requests awaiting your reply" value={requests.isPending ? '…' : String(awaiting.length)} icon={Inbox} to="/candidate/requests" highlight={awaiting.length > 0} />
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-foreground/12 bg-surface px-5 py-4">
        <p className="flex items-center gap-2 text-sm">
          ...
          {p.visible ? 'Your anonymized profile is visible to approved companies.' : 'Your profile is hidden from company search.'}
        </p>
        <Link to="/candidate/profile" className="text-sm font-semibold text-brand">Change</Link>
      </div>

      {awaiting.length > 0 && (
        <section className="mt-8" aria-labelledby="awaiting-heading">
          <h2 id="awaiting-heading" className="text-lg font-semibold tracking-[-.02em]">A company wants to talk to you</h2>
              ...
              <li key={r.id}>
                <Link to={`/candidate/requests#${r.id}`} className="flex items-center justify-between gap-4 rounded-lg border border-brand/40 bg-brand-soft/50 px-5 py-4 transition hover:border-brand">
                  <span>
                    ...
                    <span className="block text-sm text-foreground/65">{r.job?.title ?? r.roleTitle ?? 'Open role'} · received {timeAgo(r.updatedAt)}</span>
                  </span>
                  <span className="inline-flex items-center gap-1 text-sm font-semibold text-brand">Review <ArrowRight size={15} aria-hidden /></span>
                </Link>
              </li>
            ))}
          </ul>
      ...
      <section className="mt-8" aria-labelledby="recent-heading">
        <div className="flex items-center justify-between">
          <h2 id="recent-heading" className="text-lg font-semibold tracking-[-.02em]">Recent applications</h2>
          <Link to="/candidate/applications" className="text-sm font-semibold text-brand">See all</Link>
        </div>
        {applications.isPending ? (
            ...
            {applications.data.slice(0, 4).map((a) => (
              <li key={a.id} className="flex items-center justify-between gap-4 px-5 py-4">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-foreground/60">{a.job.companyName} · applied {timeAgo(a.createdAt)}</p>
                </div>
          ...
          <p className="mt-3 rounded-lg border border-dashed border-foreground/20 p-6 text-center text-sm text-foreground/60">
            You haven't applied to a position yet. <Link to="/jobs" className="font-semibold text-brand">Browse open positions</Link>
          </p>
        )}
      </section>
    </>
