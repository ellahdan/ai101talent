...
export function ShortlistDialog({ candidate, onClose }: { candidate: AnonymizedCandidate; onClose: () => void }) {
  ...
  const onCreate = (e: FormEvent) => {
    ...
    create.mutate(name.trim(), {
      onSuccess: (list) => {
        setName('')
        toggle.mutate({ listId: list.id, candidateId: candidate.id, add: true }, { onSuccess: () => toast.success(`Saved to “${list.name}”`) })
      },
    })
  }

  return (
    <Modal open onOpenChange={(o) => !o && onClose()} title="Save to shortlist" description={<>Candidate <span className="font-mono">{candidate.applicantNumber}</span> · {candidate.headline}</>}>
      {lists.isPending ? (
        <Spinner className="size-5 text-foreground/50" />
      ) : (
        <ul className="max-h-64 space-y-1 overflow-y-auto">
          {lists.data?.length === 0 && <li className="text-sm text-foreground/60">You don't have any shortlists yet. Create one below.</li>}
          {lists.data?.map((l) => {
      ...
      <form onSubmit={onCreate} className="mt-4 flex gap-2 border-t border-foreground/10 pt-4">
        <label htmlFor="new-shortlist" className="sr-only">New shortlist name</label>
        <Input id="new-shortlist" value={name} onChange={(e) => setName(e.target.value)} placeholder="New shortlist, e.g. Frontend Q4" maxLength={80} className="h-10" />
        <Button type="submit" disabled={!name.trim() || create.isPending} className="h-10 rounded-md bg-brand px-3 text-brand-foreground hover:bg-brand-hover"><Plus data-icon="inline-start" aria-hidden /> Create</Button>
      </form>
      {create.error && <Alert variant="error" className="mt-3">{create.error.message}</Alert>}
      <div className="mt-5 flex justify-end"><Button type="button" variant="outline" className="h-10 rounded-md" onClick={onClose}>Done</Button></div>
    </Modal>
  )
}
