...
export const availabilityText = (c: Pick<AnonymizedCandidate, 'availability' | 'noticePeriodWeeks'>) =>
  c.availability === 'immediately' ? tx(common).availability.now : tx(common).availability.notice(c.noticePeriodWeeks ?? '?')

interface TalentCardProps {
  candidate: AnonymizedCandidate
  /** Link to the candidate's anonymized profile. */
  href: string
...
export function TalentCard({ candidate: c, href, onSave, onRequest }: TalentCardProps) {
  const t = useT(talentText).card
  ...
  return (
    <article className="flex h-full flex-col rounded-lg border border-foreground/12 bg-surface p-5 transition hover:border-brand/50 hover:shadow-md">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="truncate font-semibold">
              <Link to={href} className="hover:text-brand">{c.headline || t.candidate}</Link>
            </h3>
            <p className="font-mono text-xs text-foreground/55">{c.applicantNumber}</p>
          </div>
        </div>
        <span className="shrink-0 rounded-full bg-chip px-2.5 py-1 text-xs font-semibold">{labels.yearsShort(c.totalYearsExperience)}</span>
      </div>

      <ul className="mt-4 space-y-1.5 text-sm text-foreground/65">
        <li className="flex items-center gap-2"><MapPin size={14} aria-hidden /> {place || t.noLocation}{c.workMode ? ` · ${labels.prefers(workModeLabel[c.workMode])}` : ''}</li>
      ...
      <div className="mt-4 flex flex-wrap gap-1.5" aria-label={t.topSkills}>
        {c.topSkills.map((s) => (
          <span key={s.name} className="rounded-full bg-brand-soft px-2.5 py-1 text-xs font-semibold text-brand-soft-foreground">
            {s.name}{s.years != null && <span className="font-normal opacity-75"> · {labels.yearsTiny(s.years)}</span>}
          </span>
        ))}
      </div>
      {c.tools.length > 0 && <p className="mt-2.5 text-xs text-foreground/55">{t.tools} {c.tools.join(', ')}</p>}
      ...
      {(c.shortlists.length > 0 || c.requestStatus) && (
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
          ...
          {c.requestStatus && <StatusBadge status={c.requestStatus} label={c.requestStatus === 'rejected' ? t.requestNotForwarded : undefined} />}
        </div>
      )}
      <div className="mt-4 flex flex-wrap gap-2 border-t border-foreground/10 pt-4">
        <Button type="button" variant="outline" className="h-9 grow rounded-md" onClick={() => onSave(c)}>
          {c.shortlists.length ? <BookmarkCheck data-icon="inline-start" aria-hidden className="text-brand" /> : <Bookmark data-icon="inline-start" aria-hidden />} {t.save}
        </Button>
        <Button type="button" className="h-9 grow rounded-md bg-brand text-brand-foreground hover:bg-brand-hover" disabled={activeRequest} onClick={() => onRequest(c)} title={activeRequest ? t.openRequest : undefined}>
          <MessageSquarePlus data-icon="inline-start" aria-hidden /> {activeRequest ? t.requested : t.request}
        </Button>
      </div>
    </article>
  )
